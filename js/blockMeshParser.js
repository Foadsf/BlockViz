...
class BlockMeshParser {
    ...
    _removeCommentsAndStoreLines(fileContent) {
        ...
        this.rawLinesWithNumbers = rawLines.map((line, index) => ({ number: index + 1, text: line }));
    ...
    _preprocessMacrosAndEvals(lines) {
        ...
        for (const line of lines) {
            this.currentLineNumber = this.rawLinesWithNumbers.find(rl => rl.text.includes(line.split(' ')[0]))?.number || this.currentLineNumber + 1; // Approximate line num
            ...
            if (match) {
                ...
                try {
                    ...
                    if (typeof result === 'number' && !isNaN(result)) {
                        ...
                        this._logError(`Failed to evaluate #eval expression for ${key}: "${expression}" did not yield a number.`);
                        this.macros[key] = NaN; // Mark as problematic
                    }
                } catch (e) {
                    this._logError(`Error evaluating #eval for ${key}: ${e.message}. Expression: "${expression}"`);
                    this.macros[key] = NaN; // Mark as problematic
                }
    ...
    parse(fileContent) {
        ...
        while (i < lines.length) {
            ...
            const originalLineEntry = this.rawLinesWithNumbers.find(rl => rl.text.includes(firstToken));
            this.currentLineNumber = originalLineEntry ? originalLineEntry.number : (this.currentLineNumber || 0) +1;
        ...
        console.log("Parsing finished. Vertices:", this.vertices.length, "Blocks:", this.blocks.length);
        ...
        while (index < lines.length && !lines[index].trim().startsWith(")")) {
            ...
            if (match) {
                ...
                if (typeof x === 'number' && typeof y === 'number' && typeof z === 'number') {
                    ...
                    this._logError(`Could not resolve all vertex coordinates to numbers: (${match[1]}, ${match[2]}, ${match[3]}) resolved to (${x}, ${y}, ${z})`);
                }
            } else if (line !== "") { // Non-empty line that doesn't match
                this._logError(`Invalid vertex format: ${line}`);
            }
            index++;
        }
        if (index < lines.length && lines[index].trim().startsWith(")")) index++; // Consume closing ); or )
        else {
    ...
    _parseEdges(lines, index) {
        ...
        while (index < lines.length && !lines[index].trim().startsWith(")")) {
            ...
            // arc v1 v2 (px py pz)
            if (parts.length >= 4 && (parts[0] === "arc" || parts[0] === "spline" || parts[0] === "polyLine" || parts[0] === "line")) {
                const type = parts[0];
                ...
                if (pointStringMatch) {
                    ...
                    if (type === "arc" && pointStringMatch.length > 0) {
                        ...
                        } else {this._logError(`Arc edge expects 3 coordinates for point, got: ${pointStringMatch[0]}`);}
                        ...
                        this._logError(`Parsing for '${type}' edge points is basic and may fail on complex structures. Line: ${line}`);
                        // Attempt basic parsing for simple lists of points
                        const allPointsStr = line.substring(line.indexOf(parts[2]) + parts[2].length).trim();
                        const pointGroups = allPointsStr.match(/\(\s*[\d.-]+\s+[\d.-]+\s+[\d.-]+\s*\)/g);
                        if (pointGroups) {
            ...
            } else if (line !== "" && line !== "(" && line !== ")") { // Check if not just parenthesis
    ...
    _parseBlocks(lines, index) {
        ...
        while (index < lines.length && !lines[index].trim().startsWith(")")) {
            ...
            if (line.startsWith("hex")) {
                ...
                let cells = null, gradingType = "simpleGrading", grading = [1,1,1]; // Defaults
                ...
                if (remainingLineParts.length > 0) {
                    for (const part of sameLineParts) {
                        const trimmedPart = part.trim();
                        if (trimmedPart.startsWith("(")) { // Cell counts
                            const cellMatch = trimmedPart.match(/^\(([^)]+)\)/);
                            if (cellMatch && !cells) {
                                cells = cellMatch[1].trim().split(/\s+/).map(s => this._resolveValue(s)).map(Number);
                            }
                        } else if (trimmedPart.startsWith("simpleGrading") || trimmedPart.startsWith("edgeGrading")) {
                            gradingType = trimmedPart.startsWith("simpleGrading") ? "simpleGrading" : "edgeGrading";
                            const gradingMatch = trimmedPart.match(/\(([^)]+)\)/);
                            if (gradingMatch) {
                                grading = gradingMatch[1].trim().split(/\s+/).map(s => this._resolveValue(s)).map(Number);
                ...
                if (!cells && lookAheadIndex < lines.length && lines[lookAheadIndex].trim().startsWith("(")) {
                ...
                if (lookAheadIndex < lines.length && (lines[lookAheadIndex].trim().startsWith("simpleGrading") || lines[lookAheadIndex].trim().startsWith("edgeGrading"))) {
                    const gradingLine = lines[lookAheadIndex].trim();
                    gradingType = gradingLine.startsWith("simpleGrading") ? "simpleGrading" : "edgeGrading";
                ...
                if (blockVertices.length !== 8) {
                    this._logError(`Hex block definition requires 8 vertices, found ${blockVertices.length}: ${hexMatch[0]}`);
                } else if (!cells || cells.length !== 3 || cells.some(isNaN)) {
                     this._logError(`Hex block requires 3 cell count numbers (nx ny nz), found: ${cells}`);
                } else if (!grading || grading.some(isNaN) || (gradingType === "simpleGrading" && grading.length !==3) /* add more for edgeGrading */) {
                     this._logError(`Invalid grading for block: type=${gradingType}, values=${grading}`);
                } else {
    ...
    _parseBoundary(lines, index) {
        ...
        while(index < lines.length && !lines[index].trim().startsWith(")")) { // Outer closing paren for boundary
            let line = lines[index].trim();

            if (currentPatch === null && line.match(/^[a-zA-Z0-9_]+$/)) { // Patch name
                ...
                if (!lines[index] || lines[index].trim() !== "(") {
                     this._logError(`Patch '${currentPatch.name}': Expected '(' after faces keyword.`);
                ...
                let faceList = [];
                while(index < lines.length && !lines[index].trim().startsWith(")")) { // faces list closing )
                    const faceLine = lines[index].trim();
                    const faceMatch = faceLine.match(/\(([^)]+)\)/);
                    if (faceMatch) {
                        const faceVertices = faceMatch[1].trim().split(/\s+/).map(Number);
                        if (faceVertices.some(isNaN) || faceVertices.length < 3) { // Typically 4 for hex faces
                            this._logError(`Patch '${currentPatch.name}': Invalid vertex numbers in face: ${faceLine}`);
                        } else {
                            faceList.push(faceVertices);
                        }
                    } else if (faceLine !== "") {
                        this._logError(`Patch '${currentPatch.name}': Malformed face entry: ${faceLine}`);
                    }
                    index++;
                }
                if (currentPatch) currentPatch.faces = faceList;
                 if (index < lines.length && lines[index].trim().startsWith(")")) { // Closing ) for faces list
                    index++; // Consume it
                    if (index < lines.length && lines[index].trim() === ";") index++; // Consume optional semicolon
                 } else {
                     this._logError(`Patch '${currentPatch.name}': Missing closing ')' for faces list.`);
                 }
            } else if (line === "}" && currentPatch) { // End of patch definition block
                if (!currentPatch.type) this._logError(`Patch '${currentPatch.name}' missing 'type' definition.`);
                if (!currentPatch.faces || currentPatch.faces.length === 0) this._logError(`Patch '${currentPatch.name}' has no faces defined or faces parsing failed.`);
    ...
    _skipToPatchEnd(lines, index) {
        ...
        while(index < lines.length) {
            ...
            if (braceLevel === 0 && currentLine.match(/^[a-zA-Z0-9_]+$/) && !lines[index+1].trim().startsWith("{")) { // Next patch name
                return index;
            }
    ...
    _parseMergePatchPairs(lines, index) {
        ...
        if (!lines[index] || lines[index].trim() !== "(") {
            ...
            if(lines[index-1].trim().startsWith("mergePatchPairs")) this._logError("Expected '(' after mergePatchPairs keyword.");
            return index;
        }
        index++;
        while(index < lines.length && !lines[index].trim().startsWith(")")) {
        ...
        if (index < lines.length && lines[index].trim().startsWith(")")) index++;
        else if(lines[index-1].trim().startsWith("mergePatchPairs")) this._logError("Missing closing ')' for mergePatchPairs block.");
        return index;
    }
}